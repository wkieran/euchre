...
use crate::{
    app::App,
    tui::components::{
        CARD_H, CARD_H_LAND, CARD_W, render_hand, render_hand_back, render_hand_back_vertical,
...
pub fn render(frame: &mut Frame, _app: &App, area: Rect) {
    ...
    let center_group_area = Layout::vertical([
        Constraint::Fill(1),
        Constraint::Length(3 + CARD_H),
        Constraint::Fill(1),
    ])
    .split(middle[1])[1];

    let center_group = Layout::vertical([Constraint::Length(3), Constraint::Length(CARD_H)])
